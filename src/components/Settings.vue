<template>
  <ion-content fullscreen class="ion-padding">
      <ion-row>
        ...
        <ion-col size="3">
          <ion-select :value="difficulty" @ionChange="changeDiff($event)" interface="popover" class="regularText" style="text-align:right"> 
...
export default {
  ...
  methods: {
    changeDiff($event) {
      ...
      if ($event.target.value == this.difficulty) return;
      this.$emit("changeDiff", $event.target.value);
